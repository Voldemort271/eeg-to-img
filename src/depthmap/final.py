...
input_dir = "output/images/04-20_03-30_vae_imgs/epoch_98"
output_dir = "output/images/results_epoch98_blurred_final"
...
for fname in os.listdir(input_dir):
    ...
    blurred_depth_image = depth_img.filter(ImageFilter.GaussianBlur(radius=6))
    blurred_depth_image.save(os.path.join(blurred_depth_map_dir, f"{idx}_blurreddepth.png"))
    # depth_img = Image.open("output_depth_map.png").convert("RGB").resize((512,512))
    # blurred_depth_image = Image.open("blurred_output_depth_map_gaussian.png").convert("RGB").resize((512,512))
    ...
    out_d = pipe_with_blurreddepth(
        prompt=prompt,
        image=blur,
        control_image=blurred_depth_image,
        num_inference_steps=20
    ).images[0]
    out_d.save(os.path.join(blurred_depth_map_dir, f"{idx}_with_blurreddepth.png"))
    m_d = calculate_metrics(orig, out_d)
    for k,v in m_d.items(): metrics_accum["with_bluureddepth"][k] += v
...
avg_with_bluureddepth={k: v / count for k, v in metrics_accum["with_bluureddepth"].items()}
...
print("-- With Blurred Depth --")
for k,v in avg_with_bluureddepth.items(): print(f"{k:7s}: {v:.4f}")
