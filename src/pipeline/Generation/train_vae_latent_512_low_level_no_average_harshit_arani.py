...
def train_model(eegmodel, imgmodel, dataloader, optimizer, device, text_features_all, img_features_all, save_dir, epoch):
    ...
    for batch_idx, (eeg_data, labels, text, text_features, img, img_features) in enumerate(dataloader):
        ...
        if img_features.dim() > 2:
            img_features_c = img_features.view(img_features.size(0), -1)
        labels = labels.to(device)
        
        optimizer.zero_grad()
        eeg_features = eegmodel(eeg_data[:, :, :250]).float()
        eeg_features_c = eeg_features.view(eeg_features.size(0), -1)
        # Use max pooling across time (dim=1) humne add kliya hai to match the shape 
        # if eeg_features.dim() == 3:
        #     eeg_features = eeg_features.max(dim=1).values  # shape becomes (B, D)
        # SupCon: joint embeddings and labels
        embeddings = torch.cat([eeg_features_c, img_features_c], dim=0)
        labels_concat = torch.cat([labels, labels], dim=0)  
        # pdb.set_trace()
        print(embeddings.shape)
        loss = supcon_loss_fn(embeddings, labels_concat) # humne label_concat ki jagah labels use kiya tha

        # weight decay
        l2_norm = sum(p.pow(2.0).sum() for p in eegmodel.parameters())
        loss = loss + ridge_lambda * l2_norm

        loss.backward()
        optimizer.step()
        total_loss += loss.item()

        # single-shot reconstruction logging
        with torch.no_grad():
            if not image_reconstructed:
                ...
                for i, label in enumerate(labels.tolist()):
                    image_rec[i].save(os.path.join(epoch_save_dir, f"reconstructed_image_{label}.png"))
                    image_train[i].save(os.path.join(epoch_save_dir, f"train_image_{label}.png"))
...
def evaluate_model(eegmodel, imgmodel, dataloader, device, text_features_all, img_features_all, k, save_dir, epoch):
    ...
    with torch.no_grad():
        for batch_idx, (eeg_data, labels, text, text_features, img, img_features) in enumerate(dataloader):
            ...
            # img_features = img_features.to(device).float()
            # labels = labels.to(device)
            # Manual SupCon batch sampling: P classes × K trials each
            P = 4    # number of classes per batch
            K = 5   # number of samples per class (your dataset has 10 trials/class)
            batch_idxs   = dataloader.dataset.get_supcon_indices(P, K)
            eeg_data     = dataloader.dataset.data[batch_idxs].to(device)
            labels       = dataloader.dataset.labels[batch_idxs].to(device)
            # bring in the matching image latents for those exact trials
            img_features = dataloader.dataset.img_features[labels.tolist()].to(device).float()
            if img_features.dim() > 2:
                img_features_c = img_features.view(img_features.size(0), -1)
            eeg_features = eegmodel(eeg_data[:, :, :250]).float()
            eeg_features_c = eeg_features.view(eeg_features.size(0), -1) 
            ...
            if epoch % 10 == 0 and batch_idx == 0:
                ...
                image_rec = image_processor.postprocess(x_rec, output_type='pil')
                for i, label in enumerate(labels.tolist()):
                    image_rec[i].save(os.path.join(epoch_save_dir, f"eval_reconstructed_{label}_{i}.png"))
...
def main_train_loop(sub, current_time, eeg_model, img_model, train_dataloader, test_dataloader, optimizer, device, 
    ...
    for epoch in range(config.epochs):
        ...
        train_save_dir = f'/DATA/deep_learning/eeg-to-img/output/images/{current_time}_vae_train_imgs'
        ...
        if (epoch +1) % 5 == 0:            
            ...
            if config.insubject==True:       
                os.makedirs(f"/DATA/deep_learning/eeg-to-img/output/models/generation-contrast/{config.encoder_type}/{sub}/{current_time}", exist_ok=True)             
                file_path = f"output/models/generation-contrast/{config.encoder_type}/{sub}/{current_time}/{epoch+1}.pth"
                torch.save(eeg_model.state_dict(), file_path)            
            else:                
                os.makedirs(f"/DATA/deep_learning/eeg-to-img/output/models/generation-contrast/across/{config.encoder_type}/{current_time}", exist_ok=True)             
                file_path = f"/DATA/deep_learning/eeg-to-img/output/models/generation-contrast/across/{config.encoder_type}/{current_time}/{epoch+1}.pth"
                torch.save(eeg_model.state_dict(), file_path)
        ...
        # Add date-time prefix to save_dir
        test_save_dir = f'/DATA/deep_learning/eeg-to-img/output/images/{current_time}_vae_imgs'
...
def main():
    ...
    parser.add_argument('--data_path', type=str, default='/DATA/deep_learning/eeg-to-img/data/weights/EEG_Image_decode/Preprocessed_data_250Hz', help='Path to data')
    parser.add_argument('--output_dir', type=str, default='/DATA/deep_learning/eeg-to-img/output/statistics/generation-contrast', help='Directory to save output results')
