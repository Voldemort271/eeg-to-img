    ...
    for epoch in range(config['epochs']):
        ...
        if epoch%5 == 0:                        
            if config['insubject']==True:                
                torch.save(model.state_dict(), f"output/models/ATME/{sub}_{epoch}.pth")
            else:
                torch.save(model.state_dict(), f"output/models/ATME/across_{epoch}.pth")
...
def main():
    ...
    for sub in subjects:
        ...
        results_dir = f"output/statistics/{args.encoder_type}/{sub}/{current_time}"
