...
#             features_filename = os.path.join(f'/DATA/deep_learning/eeg-to-img/data/weights/EEG_Image_decode/test_image_latent_512.pt') if self.train else os.path.join(f'/DATA/deep_learning/eeg-to-img/data/weights/EEG_Image_decode/test_image_latent_512.pt')
...
#                         preprocessed_eeg_data_class = preprocessed_eeg_data[start_index:start_index+samples_per_class]
#                         # print("preprocessed_eeg_data_class", preprocessed_eeg_data_class.shape)
#                         labels = torch.full((samples_per_class*80,), i, dtype=torch.long).detach()  # Add class labels # humne question kiya hai why has the array been made of length 80  samples?
...
#             # pdb.set_trace()
#             label_tensor = label_tensor.repeat_interleave(4) #humne . ye madarchod chineese likh nahi sakte the kya dhang se.ek class ki pehle 10 length ka vector banaya hai aur phir ussi ko 4 baar repeat kar diya hai 
#             if self.classes is not None:
...
#             else:
#                 text_index = (index % index_n_sub_test) // (1 * 80)   #80 se divide kiya hai humne
#             # img_index: classes * 10
#             if self.train:
#                 img_index = (index % index_n_sub_train) // (4)
#             else:
#                 img_index = (index % index_n_sub_test) // (80) #80 se divide kiya hai humne
#         else:
#             if self.classes is None:
...
#             else:
#                 text_index = (index % index_n_sub_test) // (80) #80 se divide kiya hai humne
#             # img_index: classes * 10
#             if self.train:
#                 img_index = (index % index_n_sub_train) // (4)
#             else:
...
#         # print("Text",text) Humne khud add kiya tha
        
#         # pdb.set_trace()
#         print("parameter index", index)
#         print("text_index",text_index)
#         print("length of text array",len(self.text))
#         print("Image index: ",img_index)
#         print("length of image array",len(self.img))
...
#     # 100 Hz
#     train_loader = DataLoader(train_dataset, batch_size=1, shuffle=True) #False kiya THA ise humne
#     test_loader = DataLoader(test_dataset, batch_size=1, shuffle=True) # False kiya THA ise humne
...
class EEGDataset(Dataset):
    """
    EEGDataset that loads preprocessed EEG trials, image/text metadata,
    and builds an index map for supervised contrastive sampling.
    """
    def __init__(
        self,
        data_path,
        ...
        self.subjects = self.subject_list if subjects is None else subjects
        assert any(sub in self.subject_list for sub in self.subjects), \
            f"None of {self.subjects} in data path"

        # Number of classes depends on train/test
        self.n_cls = 1654 if train else 200
        ...
        # Build label -> indices map for SupCon sampling
        self.label_to_indices = defaultdict(list)
        for idx, lbl in enumerate(self.labels.tolist()):
            self.label_to_indices[int(lbl)].append(idx)

        # Load or encode image/text features
        if self.classes is None and self.pictures is None:
            features_filename = (
                os.path.join(
                    '/DATA/deep_learning/eeg-to-img/data/weights/EEG_Image_decode',
                    'train_image_latent_512.pt' if self.train else 'test_image_latent_512.pt'
                )
            )
            if os.path.exists(features_filename):
                ...
                raise FileNotFoundError(f"Features not found at {features_filename}")
        else:
            self.text_features = self.Textencoder(self.text)
            self.img_features = self.ImageEncoder(self.img)
    ...
    def get_supcon_indices(self, num_classes, samples_per_class):
        """
        Sample a batch of indices for SupCon: pick `num_classes` random classes,
        then `samples_per_class` random trials from each class.
        Returns a flat list of dataset indices of length num_classes * samples_per_class.
        """
        chosen_classes = random.sample(list(self.label_to_indices.keys()), num_classes)
        batch_indices = []
        for cls in chosen_classes:
            batch_indices.extend(
                random.sample(self.label_to_indices[cls], samples_per_class)
            )
...
if __name__ == "__main__":
    ...
    # # EEG training data: torch.Size([16540, 4, 17, 100]) number of training images, number of repetitions per image, number of channels, EEG signal time points]
    # EEG test data: torch.Size([200, 80, 17, 100]) [number of test images, number of repetitions per image, number of channels, EEG signal time points]    # 1秒 'times': array([-0.2 , -0.19, -0.18, ... , 0.76,  0.77,  0.78, 0.79])}
    # 17个通道'ch_names': ['Pz', 'P3', 'P7', 'O1', 'Oz', 'O2', 'P4', 'P8', 'P1', 'P5', 'PO7', 'PO3', 'POz', 'PO4', 'PO8', 'P6', 'P2']
    # 100 Hz
    train_loader = DataLoader(train_dataset, batch_size=1, shuffle=True) #False kiya THA ise humne
    test_loader = DataLoader(test_dataset, batch_size=1, shuffle=True) # False kiya THA ise humne
