    ...
    for epoch in range(config.epochs):
        ...
        if (epoch +1) % 5 == 0:                    
            ...
            if config.insubject==True:       
                os.makedirs(f"output/models/retrieval-contrast/{config.encoder_type}/{sub}/{current_time}", exist_ok=True)             
                file_path = f"output/models/retrieval-contrast/{config.encoder_type}/{sub}/{current_time}/{epoch+1}.pth"
                torch.save(eeg_model.state_dict(), file_path)            
            else:                
                os.makedirs(f"output/models/retrieval-contrast/across/{config.encoder_type}/{current_time}", exist_ok=True)             
                file_path = f"output/models/retrieval-contrast/across/{config.encoder_type}/{current_time}/{epoch+1}.pth"
                torch.save(eeg_model.state_dict(), file_path)
...
def main():
    ...
    parser.add_argument('--data_path', type=str, default="data/weights/EEG_Image_decode/Preprocessed_data_250Hz", help='Path to the EEG dataset')
