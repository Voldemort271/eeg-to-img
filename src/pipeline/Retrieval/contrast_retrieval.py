    ...
    for epoch in range(config['epochs']):
        ...
        if epoch%5 == 0:                        
            if config['insubject']==True:                
                torch.save(model.state_dict(), f"output/models/retrieval-contrast/{sub}_{epoch}.pth")
            else:
                torch.save(model.state_dict(), f"output/models/retrieval-contrast/across_{epoch}.pth")
...
def main():
    ...
    parser.add_argument('--data_path', type=str, default="data/weights/EEG_Image_decode/Preprocessed_data_250Hz", help='Path to the preprocessed data')
    ...
    for sub in subjects:
        ...
        results_dir = f".output/statistics/retrieval-contrast/{args.encoder_type}/{sub}/{current_time}"
