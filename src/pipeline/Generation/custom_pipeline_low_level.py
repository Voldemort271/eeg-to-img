...
class Generator4Embeds:
    ...
    def __init__(self, num_inference_steps=1, device='cuda', img2img_strength=1, low_level_image=None, low_level_latent = None) -> None:
        ...
        self.dtype = torch.bfloat16   #humne change kiya THA isse from bfloat16 to float16
        self.device = device
        ...
        pipe = DiffusionPipeline.from_pretrained("stabilityai/sdxl-turbo", torch_dtype=torch.bfloat16, variant="fp16")#humne change kiya THA model from the stabilityai/sdxl-turbo to the stabilityai/stable-diffusion-xl-base-1.0
        # pipe = DiffusionPipeline.from_pretrained(path, torch_dtype=torch.bfloat16, variant="fp16")
        pipe.to(device)


        pipe.enable_model_cpu_offload() # Humne added this line


        pipe.generate_ip_adapter_embeds = generate_ip_adapter_embeds.__get__(pipe)
        #cuda cache clear iya hai humne
        ...
        pipe.load_ip_adapter(
            ...
            torch_dtype=torch.bfloat16) #humne change kiya THA isse from bfloat16 to float16 for the cuda of sdxl file
...
if __name__ == "__main__":
    ...
    # image_encoder = CLIPVisionModelWithProjection.from_pretrained(
    #     "h94/IP-Adapter",  latent space processing
    #     subfolder="models/image_encoder",
    #     torch_dtype=torch.bfloat16,
    # ).to("cuda")
    # feature_extractor = CLIPImageProcessor()
